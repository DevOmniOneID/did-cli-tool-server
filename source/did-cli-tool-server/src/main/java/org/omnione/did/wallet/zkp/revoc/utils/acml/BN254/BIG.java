...
package org.omnione.did.wallet.zkp.revoc.utils.acml.BN254;
...
/* return number of bits */
	...
	*  let mut res = BIG::new();
		let len = val.len();
		let op = &val[0..1];
		let n = u8::from_str_radix(op, 16).unwrap();// 16 -> 10진수
		res.w[0] += n as Chunk;
		for i in 1..len {
			res.shl(4);
			let op = &val[i..i+1];  // ch -> 10진수
	...
	*/
	/* String to BIG */
	public static BIG fromString(String val) {
		...
		op[0] = str[0];
		int n = Integer.parseInt(String.valueOf(op[0]),16);
		res.w[0] += n;

		for (int i = 1; i < len; i++) {
			res.shl(4);
			op[0] = str[i];
			n = Integer.parseInt(String.valueOf(op[0]),16);
			res.w[0] += n;
		}
		return res;
	}
