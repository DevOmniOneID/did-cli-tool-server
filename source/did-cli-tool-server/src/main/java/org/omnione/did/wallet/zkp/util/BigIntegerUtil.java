...
public class BigIntegerUtil {
    ...
    public static BigInteger createRandomInRange(BigInteger min, BigInteger max, SecureRandom random) throws ZkpException {
        ...
        if (cmp >= 0) {
            if (cmp > 0) {
                throw new ZkpException(ZkpErrorCode.OMNI_ERROR_ZKP_UNKNOWN, "'MIN' may not be greater than 'MAX'");
            }
            return min;
        }
    ...
    public static BigInteger fromBytes(byte[] bytes) throws ZkpException {
        try {
    ...
    public static byte[] asUnsignedByteArray(BigInteger value) throws ZkpException {
        try {
    ...
    public static BigInteger getHash(byte[] src) throws ZkpException {
        try {
            ...
            throw new ZkpException(ZkpErrorCode.OMNI_ERROR_ZKP_NO_SUCH_ALG, "SHA-256 ");
        } catch (Exception e) {
            throw new ZkpException(ZkpErrorCode.OMNI_ERROR_ZKP_FINAL_FOR_HASH_FAIL);
        }
    }
    ...
    public static BigInteger calc_teq(CredentialPrimaryPublicKey publicKey,
        ...
        for (String key : unrevealed_attrs) {
            ...
            if (cur_m == null)
                throw new ZkpException(ZkpErrorCode.OMNI_ERROR_ZKP_NULL, "Value by key '{}' not found in m_tilde ");
...
//
//        System.out.println("m2_tilde: "+m2_tilde);
//
//        BigInteger cur_r, cur_m;
//
//        for (String key : unrevealed_attrs) {
...
//    }
    ...
    public static Vector<BigInteger> calc_tne(CredentialPrimaryPublicKey p_pub_key,
        ...
        for (int i = 0; i < ZkpConstants.ITERATION; i++) {
            ...
            if (cur_u == null)
                throw new ZkpException(ZkpErrorCode.OMNI_ERROR_ZKP_NULL, "Value by key [" + i + "] not found in u");

            BigInteger cur_r = r.get(Integer.toString(i));
            if (cur_r == null)
                throw new ZkpException(ZkpErrorCode.OMNI_ERROR_ZKP_NULL, "Value by key [" + i + "] not found in r");
        ...
        if (delta == null)
            throw new ZkpException(ZkpErrorCode.OMNI_ERROR_ZKP_NULL, "Value by key DELTA not found in r");
        ...
        for (int i = 0; i < ZkpConstants.ITERATION; i++) {
            ...
            if (cur_t == null)
                throw new ZkpException(ZkpErrorCode.OMNI_ERROR_ZKP_NULL, "Value by key [" + i + "] not found in t");
...
//    }
    ...
    public static int[] four_squares(int delta) throws ZkpException {
        if (delta < 0)
            throw new ZkpException(ZkpErrorCode.OMNI_ERROR_ZKP_UNKNOWN, "Cannot express a negative number as sum of four squares" + delta);
    ...
    public BigInteger generateMasterSecret() throws ZkpException {
        try {
            return createRandomBigInteger(ZkpConstants.LARGE_MASTER_SECRET, this.random);
        } catch (Exception e) {
            throw new ZkpException(ZkpErrorCode.OMNI_ERROR_ZKP_PROVER_GENERATE_MASTER_SECRET_FAIL);
        }
    }
}
