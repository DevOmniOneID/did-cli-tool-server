...
public class TransactionHeader implements EosType.Packer {
	...
	@Expose
	private long max_net_usage_words = 0; // fc::unsigned_int

	@Expose
	private long max_cpu_usage_ms = 0; // fc::unsigned_int

	@Expose
	private long delay_sec = 0; // fc::unsigned_int
	...
	public void setReferenceBlock(String refBlockIdAsSha256) {
		ref_block_num = new BigInteger(1,
				HexUtils.toBytes(refBlockIdAsSha256.substring(0, 8))).intValue();

		ref_block_prefix = // new BigInteger( 1, HexUtils.toBytesReversed(
							// refBlockIdAsSha256.substring(16,24))).longValue();
				BitUtils.uint32ToLong(
						HexUtils.toBytes(refBlockIdAsSha256.substring(16, 24)), 0); // BitUtils
																					// treats
																					// bytes
																					// in
																					// little
...
//			e.printStackTrace();
	...
	public void pack(EosType.Writer writer) {
		writer.putIntLE((int) (getExpirationAsDate(expiration).getTime() / 1000)); // ms
																					// ->
																					// sec
